...
class TableSceneBuilder_tool(SceneBuilder):
    def build(self):
        builder = self.scene.create_actor_builder()
        model_dir = Path(osp.dirname(__file__)) / "assets"
        table_model_file = str(model_dir / "table.glb")
    ...
    def initialize(self, env_idx: torch.Tensor):
        ...
        b = len(env_idx)
        self.table.set_pose(
            sapien.Pose(p=[-0.12, 0, -0.9196429], q=euler2quat(0, 0, 0))
        )
        if self.env.robot_uids == "panda":
            qpos = np.array(
                [
...
class noTableSceneBuilder_microwave(SceneBuilder):
    def build(self):
        pass
    ...
    def initialize(self, env_idx: torch.Tensor):
        ...
        if self.env.robot_uids == "panda":
            ...
            # fmt: off
            qpos = np.array(
                [0.0, np.pi / 8, 0, -np.pi * 5 / 8, 0, np.pi * 3 / 4, - 3 * np.pi / 4, 0.04, 0.04]
            )
            # fmt: on
            if self.env._enhanced_determinism:
                qpos = (
