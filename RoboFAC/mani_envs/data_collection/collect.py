...
import os
from utiles import data_fix, filter_data
...
from mani_skill.examples.motionplanning.panda.error_solutions import solveStackCube_error, solvePullCubeTool_error, solvePlugCharger_error, solvePushCube_error, solvePullCube_error

from task_generalization.solve_PickCube import solve_pickcube_apple, solve_pickcube_can, solve_pickcube_ball, solve_pickcube_rubiks, solve_pickcube_lego, solve_pickcube_lock
from task_generalization.solve_StackCube import solve_stackcube_appleplate, solve_stackcube_can, solve_stackcube_lego, solve_stackcube_orangebowl, solve_stackcube_dicebrick
from task_generalization.solve_PushCube import solve_pushcube_box, solve_pushcube_cup, solve_pushcube_toy
from task_generalization.solve_PullCube import solve_pullcube_lego, solve_pullcube_scissor, solve_pullcube_block
from task_generalization.solve_PullCubeTool import solve_pullcubetool_golf, solve_pullcubetool_dice, solve_pullcubetool_can, solve_pullcubetool_peach, solve_pullcubetool_marble
from task_generalization.solve_LiftPegUpright import solve_liftpegupright_box, solve_liftpegupright_can, solve_liftpegupright_cup
# from task_generalization.solve_PegInsertionSide import solve_peginsertionside_screwdriv
from error_solutions.StackCube_v1 import solve_with_errors as solveStackCubeV1_error
# from .error_solutions import solveStackCube_error_appleplate, solve_stackcube_error_orangebowl, solve_stackcube_error_dicebrick, solve_stackcube_error_lego, solveStackCube_error_can
from mani_skill.examples.motionplanning.panda.generalization.generalization.error_solutions import (
    solveStackCube_error_appleplate,
    solve_stackcube_error_orangebowl,
    solve_stackcube_error_dicebrick,
    solve_stackcube_error_lego,
    solveStackCube_error_can,

    solvePushCube_error_box,
    solvePushCube_error_cup,
    solvePushCube_error_toy,

    solvePullCube_error_block,
    solvePullCube_error_lego,
    solvePullCube_error_scissor,

    solveTool_error_can,
    solveTool_error_dice,
    solveTool_error_golf,
    solveTool_error_marble,
    solveTool_error_peach,
)

MP_SOLUTIONS = {
    "StackCube-v1": solveStackCubeV1_error,
    "StackCube-appleplate": solveStackCube_error_appleplate,
    "StackCube-can": solveStackCube_error_can,
    "StackCube-lego": solve_stackcube_error_lego,
    "StackCube-orangebowl": solve_stackcube_error_orangebowl,
    "StackCube-dicebrick": solve_stackcube_error_dicebrick,

    "PushCube-v1": solvePushCube_error,
    "PushCube-box": solvePushCube_error_box,
    "PushCube-cup": solvePushCube_error_cup,
    "PushCube-toy": solvePushCube_error_toy,

    "PullCube-v1": solvePullCube_error,
    "PullCube-lego": solvePullCube_error_lego,
    "PullCube-scissor": solvePullCube_error_scissor,
    "PullCube-block": solvePullCube_error_block,

    "PullCubeTool-v1": solvePullCubeTool_error,
    "PullCubeTool-golf": solveTool_error_golf,
    "PullCubeTool-dice": solveTool_error_dice,
    "PullCubeTool-can": solveTool_error_can,
    "PullCubeTool-peach": solveTool_error_peach,
    "PullCubeTool-marble": solveTool_error_marble,

    "PlugCharger-v1": solvePlugCharger_error,
    # "PushCube-v1": solvePushCube_error,
    # "PullCube-v1": solvePullCube_error,
...
def _main(args, proc_id: int = 0, start_seed: int = 0) -> str:
    ...
    while True:
        try:
            res, unique_id = solve_error(env, log_file=f"{output_dir[0]}/{args.json_file}", error_stage=args.error_stage, perturbation_type=args.perturbation_type, seed=seed, debug=False, vis=True if args.vis else False)
        ...
        if args.only_count_success and not success:
            ...
            env.flush_trajectory(unique_id=unique_id)
            if args.save_video:
                env.flush_video(name=f"{unique_id}")
            pbar.update(1)
            pbar.set_postfix(
                dict(
        ...
        if collect_correct == True:
            env_id = args.env_id
            env_correct = gym.make(
                env_id,
                obs_mode=args.obs_mode,
                control_mode="pd_joint_pos",
                render_mode=args.render_mode,
            ...
            correct_traj_name = f"{new_traj_name}_correct_{unique_id}"
            env_correct = RecordEpisode(
                env_correct,
                output_dir=osp.join(args.record_dir, env_id, args.stage_dir, "_correct"), ## change the output_dir for correct data
                trajectory_name=correct_traj_name, save_video=args.save_video,
                source_type="motionplanning",
                source_desc="official motion planning solution from ManiSkill contributors",
                video_fps=30,
                save_on_reset=False
            )
            solve_correct = CORRECT_SOLUTIONS[env_id]
            try:
                res = solve_correct(env_correct,seed=seed, debug=False, vis=True if args.vis else False)
            except Exception as e:
                print(f"Cannot find valid solution because of an error in motion planning solution: {e}")
                res = -1

            if res != -1:
                env_correct.flush_trajectory(unique_id=unique_id)
                if args.save_video:
                    env_correct.flush_video(name=f"{unique_id}")
    ...
    log_file = f"{output_dir[0]}/{args.json_file}"
    input_dir = f"{output_dir[0]}"
    output_dir = f"{output_dir[0]}/_pick"
    data_fix(log_file)
    # filter_data(input_folder=input_dir, output_folder=output_dir, success_level="critical failure")
    env.close()
    if collect_correct == True:
        env_correct.close()
