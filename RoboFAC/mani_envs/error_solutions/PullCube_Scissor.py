...
ERROR_DESCRIPTIONS = get_error_descriptions(object="scissor")
...
def solve_pullcube_scissor(env: PullCubeEnv_scissor, seed=None, debug=False, vis=False):
    ...
    env = env.unwrapped
    planner.close_gripper()
    reach_pose = sapien.Pose(p=env.obj.pose.sp.p + np.array([0.12, 0, 0]), q=env.agent.tcp.pose.sp.q) # 修改位置
    planner.move_to_pose_with_screw(reach_pose)
...
if __name__ == '__main__':
    # quickstart
    env = gym.make(
        "PullCube-scissor", # there are more tasks e.g. "PushCube-v1", "PegInsertionSide-v1", ...
        num_envs=1,
    ...
    for seed in range(5):
            res = solve_with_errors(env,log_file='reach',error_stage='pull', seed=seed, debug=False, vis=True)
            time.sleep(1)
    print(res[-1])
    env.close()