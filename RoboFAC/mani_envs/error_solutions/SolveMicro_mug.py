...
def add_perturbation(task_stage, pose, gripper_state=None, perturbation_type=None):
    ...
    if task_stage == "spoon_reach":
        ...
        error_description["original_pose"] = pose.p.tolist()
        pose.p += perturbation
        error_description["stage"] = "mug_reach"
        error_description["error_type"] = "position offset"
        ...
        error_description["details"] = f"The spoon was not placed into the {objectB} before putting the {objectB} into the microwave, causing the task to fail."
        error_description["correction_suggestion"] = f"Place the spoon into the {objectB} before putting the {objectB} into the microwave."
        ...
        if perturbation_type == "position_offset":
            ...
            error_description["original_pose"] = pose.p.tolist()
            pose.p += perturbation
            error_description["stage"] = "mug_grasp"
            error_description["error_type"] = "position_offset"
            error_description["details"] = f"The robot arm did not reach a suitable position above the {objectB} during the reach stage, causing an improper and unstable grasp of the {objectB} in the grasp stage. This misalignment led to the spoon being dropped and the subsequent failure of the task.",
        ...
        error_description["error_type"] = "position offset"
        error_description["details"] = f"The spoon was not aligned during the insertion stage, causing it to fail to be placed into the {objectB}, ultimately leading to the task failure."
        error_description["correction_suggestion"] = f"Adjust the alignment of the spoon to ensure it is positioned over the {objectB} opening before attempting to place it into the {objectB}."
...
def solve_with_errors(env: MicrowaveTaskEnv, log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    ...
    if not error_stage == "forget_spoon": 
        # Reach
    ...
    if error_stage == "mug_reach":
        reach_pose, _, error_description, _ = add_perturbation(error_stage, reach_pose)
    planner.move_to_pose_with_screw(reach_pose)
    grasp_pose2 = grasp_pose * sapien.Pose([-0.05, 0.0, 0.05])
    if error_stage == "mug_grasp":
