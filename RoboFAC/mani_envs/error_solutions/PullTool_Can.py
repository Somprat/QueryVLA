...
ERROR_DESCRIPTIONS = get_error_descriptions(objectA="L-shape tool", objectB="can")
...
def solve_with_errors(env: PullCubeToolEnv_can, log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    # if error_stage == "none":
    #     solve_pullcubetool_can(env, seed=seed, debug=debug, vis=vis)
    #     return
    error_description = {}
    env.reset(seed=seed)
    logs = []
...
if __name__ == '__main__':
    # quickstart
    env = gym.make(
        "PullCubeTool-can-more", # there are more tasks e.g. "PushCube-v1", "PegInsertionSide-v1", ...
        num_envs=1,
