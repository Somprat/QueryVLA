...
def add_perturbation(task_stage, pose, gripper_state=None, perturbation_type=None):
    ...
    objectA = "goldbar"
    objectB = "safe"
    if task_stage == "goldbar_reach":
        gripper_state = -0.40
        perturbation1 = np.random.uniform(-0.07, -0.05, size=3)  # Random position offset
        perturbation2 = np.random.uniform(0.05, 0.07, size=3)  # Random position offset
        perturbations = np.concatenate((perturbation1, perturbation2))
        perturbation = np.random.choice(perturbations, size=3)
        pose.p += perturbation
        error_description["stage"] = "goldbar_reach"
        ...
        perturbation1 = np.random.uniform(-0.09, -0.07, size=3)  # Random position offset
        perturbation2 = np.random.uniform(0.07, 0.09, size=3)  # Random position offset
        perturbations = np.concatenate((perturbation1, perturbation2))
        perturbation = np.random.choice(perturbations, size=3)
        pose.p += perturbation
    ...
    elif task_stage == "goldbar_push":
        perturbation1 = np.random.uniform(-0.07, -0.05, size=3)  # Random position offset
        perturbation2 = np.random.uniform(0.05, 0.07, size=3)  # Random position offset
        perturbations = np.concatenate((perturbation1, perturbation2))
        perturbation = np.random.choice(perturbations, size=3)
        pose.p += perturbation
        error_description["stage"] = "goldbar_push"
        error_description["error_type"] = "position offset"
        ...
        if perturbation_type == "position_offset":
            ...
            gripper_state = -0.6
            pose.p += perturbation
            error_description["stage"] = "goldbar_grasp"
...
def solve_with_errors(env: SafeTaskEnv, log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    ...
    if error_stage == "goldbar_reach":
        reach_pose, gripper_state, error_description, speed_factor = add_perturbation("goldbar_reach", reach_pose, perturbation_type=perturbation_type)
        # print("hi")
    res = planner.move_to_pose_with_screw(reach_pose)
    if res == -1: return res
    # Grasp
    if error_stage == "goldbar_grasp":
        grasp_pose, gripper_state, error_description, speed_factor = add_perturbation("goldbar_grasp", grasp_pose, perturbation_type=perturbation_type)
    res = planner.move_to_pose_with_screw(grasp_pose)
    if res == -1: return res
    planner.close_gripper(gripper_state=gripper_state)
    # Lift to safe height
    lift_height = 0.2
    if error_stage == "goldbar_lift":
        lift_height = 0.05
        error_description["stage"] = "goldbar_lift"
    ...
    if error_stage == "goldbar_push":
        push_pose, gripper_state, error_description, speed_factor = add_perturbation("goldbar_push", push_pose, perturbation_type=perturbation_type)
