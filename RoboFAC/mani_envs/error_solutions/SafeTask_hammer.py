...
def add_perturbation(task_stage, pose, gripper_state=None, perturbation_type=None):
    ...
    if task_stage == "hammer_reach":
        ...
        error_description["stage"] = "hammer_reach"
        ...
        error_description["stage"] = "hammer_push"
        error_description["error_type"] = "position offset"
        ...
        if perturbation_type == "position_offset":
            ...
            error_description["stage"] = "hammer_grasp"
...
def solve_with_errors(env: SafeTaskEnv, log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    ...
    if error_stage == "hammer_reach":
        reach_pose, gripper_state, error_description, speed_factor = add_perturbation("hammer_reach", reach_pose, perturbation_type=perturbation_type)
        # print("hi")
    res = planner.move_to_pose_with_screw(reach_pose)
    if res == -1: return res
    # Grasp
    if error_stage == "hammer_grasp":
        grasp_pose, gripper_state, error_description, speed_factor = add_perturbation("hammer_grasp", grasp_pose, perturbation_type=perturbation_type)
    res = planner.move_to_pose_with_screw(grasp_pose)
    if res == -1: return res
    planner.close_gripper(gripper_state=gripper_state)
    # Lift to safe height
    lift_height = 0.2
    if error_stage == "hammer_lift":
        lift_height = 0.05
        error_description["stage"] = "hammer_lift"
    ...
    if error_stage == "hammer_push":
        push_pose, gripper_state, error_description, speed_factor = add_perturbation("hammer_push", push_pose, perturbation_type=perturbation_type)
...
if __name__ == "__main__":
    env = gym.make(
        "SafeTask-hammer", # there are more tasks e.g. "PushCube-v1", "PegInsertionSide-v1", ...
        num_envs=1,
    ...
    for seed in range(10):
        solve_with_errors(env, "log_test", "hammer_reach", perturbation_type="position_offset", seed=seed, vis=True)