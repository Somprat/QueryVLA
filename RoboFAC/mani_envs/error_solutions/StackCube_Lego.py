...
ERROR_DESCRIPTIONS = get_error_descriptions(objectA="lego", objectB="cube")
...
def add_perturbation(task_stage, pose, gripper_state=None, perturbation_type=None):
    ...
    if task_stage == "reach":
        ...
        perturbation = np.zeros(3)
        perturbation[2] = np.random.uniform(-0.17, -0.16)
        # height_offset = 0.02
        perturbation_magnitude = np.linalg.norm(perturbation)
        if perturbation_magnitude > 0:
            direction_vector = perturbation / perturbation_magnitude
...
def solve_with_errors(env: StackCubeEnv_lego, log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    logs = []
    success_level = ""
...
if __name__ == '__main__':
    # quickstart
    env = gym.make(
        "StackCube-lego", # there are more tasks e.g. "PushCube-v1", "PegInsertionSide-v1", ...
        num_envs=1,
    ...
    for seed in range(10):
        ...
        # res = solve_stackcube_lego(env,vis=True)
        print(res)
    #   break
    # seed = 5
    # p = 0
