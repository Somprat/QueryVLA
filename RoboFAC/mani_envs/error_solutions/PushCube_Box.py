...
ERROR_DESCRIPTIONS = get_error_descriptions(object="box")
...
def solve_with_errors(env: PushCubeEnv_box,log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    logs = []
    success_level = ""
...
def solve_pushcube_box(env: PushCubeEnv_box, seed=None, debug=False, vis=False):
    ...
    env = env.unwrapped
    planner.close_gripper()
    reach_pose = sapien.Pose(p=env.obj.pose.sp.p + np.array([-0.1, 0, 0]), q=env.agent.tcp.pose.sp.q) # 修改位置
    planner.move_to_pose_with_screw(reach_pose)
...
if __name__ == '__main__':
    # quickstart
    env = gym.make(
        "PushCube-box", # there are more tasks e.g. "PushCube-v1", "PegInsertionSide-v1", ...
        num_envs=1,
