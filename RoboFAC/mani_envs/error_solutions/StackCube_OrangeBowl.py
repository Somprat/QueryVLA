...
ERROR_DESCRIPTIONS = get_error_descriptions(objectA="orange", objectB="bowl")
...
def add_perturbation(task_stage, pose, gripper_state=None, perturbation_type=None):
    ...
    if task_stage == "reach":
        ...
        perturbation = np.zeros(3)
        perturbation[2] = np.random.uniform(-0.85, -0.75)
        # height_offset = 0.02
        perturbation_magnitude = np.linalg.norm(perturbation)
        if perturbation_magnitude > 0:
            direction_vector = perturbation / perturbation_magnitude
        ...
        for i in range(3):
            if np.random.rand() > 0.5:
                perturbation[i] = np.random.uniform(-0.06, -0.04)
            else:
                perturbation[i] = np.random.uniform(0.04, 0.06)
        # perturbation = np.random.uniform(-0.12, -0.08, size=3)  
...
def solve_with_errors(env: StackCubeEnv_orangebowl, log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    logs = []
    success_level = ""
    ...
    if error_stage == 'reach':
        ...
        success_level = "critical failure"
        logs.append("In front of the robot arm, there were a tomato_soup_can, a tuna_fish_can, a pear, and a potted_meat_can. The task was to grasp the pear and stack it on the tuna_fish_can, but the robot arm chose to grasp the tomato_soup_can, leading to the task's final failure.")
        error_description["stage"] = "grasp"
...
##（行数）：85,91; 172,178; 257,263; 340,346; 425,431
...
if __name__ == '__main__':
    # quickstart
    env = gym.make(
        "StackCube-orangebowl-more", # there are more tasks e.g. "PushCube-v1", "PegInsertionSide-v1", ...
        num_envs=1,
    ...
    for seed in range(10):
        res = solve_with_errors(env, log_file=log_file,error_stage='none',perturbation_type="gripper_error", seed=seed,vis=True)
        # res = solve_stackcube_orangebowl(env,vis=True)
        print(res)
    #   break
    # seed = 5
    # p = 0
