...
from tasks import MicrowaveTaskEnv
import uuid
import json
...
def add_perturbation(task_stage, pose, gripper_state=None, perturbation_type=None):
    ...
    if task_stage == "spoon_reach":
        ...
        error_description["details"] = f"The robot arm did not align perfectly over {objectA}, causing a misalignment that prevented the robot from successfully reaching the position above the {objectA}. This misalignment can lead to further errors in subsequent stages of the task. Specifically, the misalignment cause the robot grasp the spoon in a suboptimal position, which could result in the spoon being dropped in later stages of the task."
    ...
    elif task_stage == "forget_spoon":
        error_description["stage"] = "forget_spoon"
        error_description["error_type"] = "step_omission"
        error_description["details"] = "The spoon was not placed into the cup before putting the cup into the microwave, causing the task to fail."
        error_description["correction_suggestion"] = "Place the spoon into the cup before putting the cup into the microwave."
    
    elif task_stage == "spoon_grasp":
        if perturbation_type == "position_offset":
            perturbation = np.random.uniform(-0.05, 0.05, size=3)  # Random position offset
            # perturbation = np.array([0.05, 0, 0])
            gripper_state = -0.6
            pose.p += perturbation
            error_description["stage"] = "spoon_grasp"
            ...
            error_description["root_cause"] = "The gripper did not grasp the cube tightly during the grasping stage."
        ...
        if perturbation_type == "position_offset":
            ...
            error_description["error_type"] = "position_offset"
            error_description["details"] = "The robot arm did not reach a suitable position above the cup during the reach stage, causing an improper and unstable grasp of the cup in the grasp stage. This misalignment led to the spoon being dropped and the subsequent failure of the task.",
            ...
            error_description["stage"] = "cup_grasp"
            error_description["error_type"] = "gripper_error"
            error_description["details"] = "The gripper did not close tightly enough to grasp the cup, leading to the failure of subsequent tasks. This insufficient grip can cause the cup to slip or be dropped during later stages, ultimately resulting in task failure."
            error_description["correction_suggestion"] = f"Increase the gripper's grasping force when reaching the grasp position to grasp the {objectA}."
            # error_description["details"] = "Gripper closed after passing cubeA."

    elif task_stage == "spoon_upright":
        perturbation = np.random.uniform(-0.05, 0.05, size=3)  # Random position offset
        pose.p += perturbation
        error_description["stage"] = "spoon_upright"
    ...
    elif task_stage == "spoon_align":
        perturbation = np.random.uniform(-0.05, 0.05, size=3)  # Random position offset
        pose.p += perturbation
        error_description["stage"] = "spoon_align"
        error_description["error_type"] = "position offset"
        error_description["details"] = f"The spoon was not aligned during the insertion stage, causing it to fail to be placed into the cup, ultimately leading to the task failure."
        error_description["correction_suggestion"] = f"Adjust the alignment of the spoon to ensure it is positioned over the cup opening before attempting to place it into the cup."

    elif task_stage == "microwave_open":
        error_description["stage"] = "microwave_open"
        error_description["error_type"] = "position offset"
        error_description["details"] = "The microwave door was not opened completely, causing the cup to collide with the door and ultimately can not put the cup in the microwave, leading to the task failure."
        error_description["correction_suggestion"] = "Ensure the microwave door is fully open before attempting to place the cup inside."
    
    elif task_stage == "microwave_close":
        error_description["stage"] = "microwave_close"
        error_description["error_type"] = "step_omission"
        error_description["details"] = "The microwave door was not closed after putting the cup inside, causing the task to fail."
        error_description["correction_suggestion"] = "Close the microwave door after putting the cup inside."
...
def solve_with_errors(env: MicrowaveTaskEnv, log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    ...
    if not error_stage == "forget_spoon": # 如果不是忘记勺子就执行
        # Reach
        reach_pose = grasp_pose * sapien.Pose([0, 0, -0.05])
        if error_stage == "spoon_reach":
            reach_pose, _, error_description, _ = add_perturbation(error_stage, reach_pose)
        planner.move_to_pose_with_screw(reach_pose)
        # Grasp
        if error_stage == "spoon_grasp":
    ...
    # -----------------------------------------------  
        ...
        _,_, error_description,_ = add_perturbation("forget_spoon", grasp_pose)
        lift_pose = sapien.Pose([0, 0, 0.2]) * grasp_pose
        lift_pose2 = lift_pose * sapien.Pose([0.18, 0, -0.1])
        theta = np.pi * 0.225
        rotation_quat = np.array([np.cos(theta), 0, np.sin(theta), 0])  
...
if __name__ == "__main__":
    ...
    for seed in range(10):
        solve_with_errors(env, seed=seed,log_file='long_test', error_stage="forget_spoon", debug=False, vis=True)