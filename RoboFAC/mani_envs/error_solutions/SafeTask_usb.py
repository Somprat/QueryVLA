...
def add_perturbation(task_stage, pose, gripper_state=None, perturbation_type=None):
    ...
    if task_stage == "usb_reach":
        ...
        error_description["stage"] = "usb_reach"
        ...
        error_description["stage"] = "usb_push"
        error_description["error_type"] = "position offset"
        ...
        if perturbation_type == "position_offset":
            ...
            error_description["stage"] = "usb_grasp"
...
def solve_with_errors(env: SafeTaskEnv, log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    ...
    if error_stage == "usb_reach":
        reach_pose, gripper_state, error_description, speed_factor = add_perturbation("usb_reach", reach_pose, perturbation_type=perturbation_type)
        # print("hi")
    res = planner.move_to_pose_with_screw(reach_pose)
    if res == -1: return res
    # Grasp
    if error_stage == "usb_grasp":
        grasp_pose, gripper_state, error_description, speed_factor = add_perturbation("usb_grasp", grasp_pose, perturbation_type=perturbation_type)
    res = planner.move_to_pose_with_screw(grasp_pose)
    if res == -1: return res
    planner.close_gripper(gripper_state=gripper_state)
    # Lift to safe height
    lift_height = 0.2
    if error_stage == "usb_lift":
        lift_height = 0.05
        error_description["stage"] = "usb_lift"
    ...
    if error_stage == "usb_push":
        push_pose, gripper_state, error_description, speed_factor = add_perturbation("usb_push", push_pose, perturbation_type=perturbation_type)
...
if __name__ == "__main__":
    env = gym.make(
        "SafeTask-usb", # there are more tasks e.g. "PushCube-v1", "PegInsertionSide-v1", ...
        num_envs=1,
