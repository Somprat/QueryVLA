...
from tasks import MicrowaveTaskEnv, MicrowaveTaskEnv_fork, MicrowaveTaskEnv_mug, MicrowaveTaskEnv_knife
...
def add_perturbation(task_stage, pose, gripper_state=None, perturbation_type=None):
    ...
    if task_stage == "fork_reach":
    ...
    elif task_stage == "forget_fork":
        error_description["stage"] = "forget_fork"
        error_description["error_type"] = "step_omission"
        error_description["details"] = f"The fork was not placed into the {objectB} before putting the {objectB} into the microwave, causing the task to fail."
        error_description["correction_suggestion"] = f"Place the fork into the {objectB} before putting the {objectB} into the microwave."
    
    elif task_stage == "fork_grasp":
        if perturbation_type == "position_offset":
            ...
            error_description["original_pose"] = pose.p.tolist()
            pose.p += perturbation
            error_description["stage"] = "fork_grasp"
        ...
        if perturbation_type == "position_offset":
            ...
            error_description["error_type"] = "position_offset"
            error_description["details"] = f"The robot arm did not reach a suitable position above the {objectB} during the reach stage, causing an improper and unstable grasp of the {objectB} in the grasp stage. This misalignment led to the fork being dropped and the subsequent failure of the task.",
        ...
        error_description["stage"] = "fork_align"
        error_description["error_type"] = "position offset"
        error_description["details"] = f"The fork was not aligned during the insertion stage, causing it to fail to be placed into the {objectB}, ultimately leading to the task failure."
        error_description["correction_suggestion"] = f"Adjust the alignment of the fork to ensure it is positioned over the {objectB} opening before attempting to place it into the {objectB}."
...
def solve_with_errors(env: MicrowaveTaskEnv, log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    ...
    # -----------------------------------------------  
        ...
        _,_, error_description,_ = add_perturbation("forget_fork", grasp_pose)
        lift_pose = sapien.Pose([0, 0, 0.2]) * grasp_pose
        lift_pose2 = lift_pose * sapien.Pose([0.18, 0, -0.1])
        theta = np.pi * 0.225
        rotation_quat = np.array([np.cos(theta), 0, np.sin(theta), 0])  
...
if __name__ == "__main__":
    env = gym.make(
        "MicrowaveTask-fork", # there are more tasks e.g. "PushCube-v1", "PegInsertionSide-v1", ...
        num_envs=1,
    ...
    for seed in range(10):
        solve_with_errors(env, seed=seed,log_file='long_test', error_stage="fork_reach", debug=False, vis=True)