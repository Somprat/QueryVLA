...
ERROR_DESCRIPTIONS = get_error_descriptions(objectA="L-shape tool", objectB="dice")
...
def solve_with_errors(env: PullCubeToolEnv_dice, log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    # if error_stage == "none":
    #     solve_pullcubetool_dice(env, seed=seed, debug=debug, vis=vis)
    #     return
    error_description = {}
    env.reset(seed=seed)
    logs = []
...
def solve_pullcubetool_dice(env: PullCubeToolEnv_dice, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    planner = PandaArmMotionPlanningSolver(
        env,
        debug=debug,
...
if __name__ == '__main__':
    # quickstart
    env = gym.make(
        "PullCubeTool-dice", # there are more tasks e.g. "PushCube-v1", "PegInsertionSide-v1", ...
        num_envs=1,
    ...
    for seed in range(10):
        ...
        res = solve_with_errors(env,log_file='new',error_stage='reach',perturbation_type="position_offset", seed=seed, debug=False, vis=False)
        # print(res)
        # env.render()
