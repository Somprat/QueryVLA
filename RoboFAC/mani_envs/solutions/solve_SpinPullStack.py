...
from tasks.task_SpinPullStack import *
...
def main(): 
    ...
    for seed in range(10):
        solveRotation2(env, seed=seed, debug=False, vis=True)

def solveRotation2(env, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    for i in range(1000):
        ...
        if (env.cubeB.pose.sp.p[1] - env.disk_y) < 0:
            angle = np.pi + angle
        if angle > (np.pi * 1.5 - 0.08) and angle < (np.pi * 1.5 - 0.02):
            planner.open_gripper()
            break
...
def solveRotation2_gen1(env, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    assert env.unwrapped.control_mode in [
        "pd_joint_pos",
        "pd_joint_pos_vel",
...
def solveRotation2_gen2(env, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    assert env.unwrapped.control_mode in [
        "pd_joint_pos",
        "pd_joint_pos_vel",
